import { Controller, Get, Param, NotFoundException, Post, UploadedFile, UseInterceptors, BadRequestException, Body, Put, Delete } from '@nestjs/common';
...
export class ProductosController {
  ...
  async crearProducto(@Body() body: any, @UploadedFile() file: any) {
    if (file) {
      body.Imagen = `/imagenes/${file.filename}`; // Establecer ruta de la imagen
    }
  ...
  @Put(':id')
  async actualizarProducto(@Param('id') id: string, @Body() body: any, @UploadedFile() file: any) {
    const idNumerico = parseInt(id, 10);
    if (isNaN(idNumerico)) {
      throw new BadRequestException('El ID debe ser un número válido');
    }
    ...
    if (file) {
      body.Imagen = `/imagenes/${file.filename}`; // Establecer nueva ruta de imagen si se sube
    }
  ...
  async eliminarProducto(@Param('id') id: string) {
    ...
    const productoExistente = await this.productosService.obtenerProductoPorId(idNumerico);
  ...
  async subirImagen(@UploadedFile() file: any) {
    return { ruta: `/imagenes/${file.filename}` };
  }
